...
int main(int argc, char *argv[])
{
    ...
    qDebug() << "plugin dir:" << qApp->libraryPaths();
    ...
    for (int x = 0; x < argc; x++) {
        ...
        } else if (0 == strcmp(argv[x], "-useGerman")) {
            // Client st in Englisch
            //Settings::getInstance()->setIsLanguageGerman(false);
            Settings::getInstance()->setIsLanguageGerman(true);
    ...
    */
    ...
    if (!QSystemTrayIcon::isSystemTrayAvailable()) {
        Message::error(QObject::tr("No systray found on this system."), QObject::tr("NTUMEMS OpenVPN"));
        return 1;
    }
